...
<div id="{{$input->vueElement()}}" class="{{$input->containerClasses()}}" style="clear: both">
    ...
    <table id="partsTable" class="table">
        <tr v-for="(part, i) in parts" :key="i">
            <td style="width: 20%">
                <button type="button" v-on:click="removeRow(i)" class="btn btn-default btn-sm remove-package pull-left"><i class="fa fa-close"></i></button>
            ...
            <td>
                        <textarea type="text" :name="'parts['+i+'][value]'" v-model="part.value"
                                  class="validate[required] form-control " placeholder="value"></textarea>
    ...
    @include('form.hidden',['var'=>['name'=>$input->name,'label'=>$input->label, 'params'=>['v-model'=>'partsString'],'value'=>($input->value() ?? '[]')]])
</div>
...
@section('js')
    ...
    <script>
        ...
        var {{$input->vueElement()}} = new Vue({
            ...
            methods: {
                addRow: function () {
                    this.parts.push({name: 'key', content: 'value'});
                },
